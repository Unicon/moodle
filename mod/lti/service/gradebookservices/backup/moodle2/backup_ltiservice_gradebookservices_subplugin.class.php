...
class backup_ltiservice_gradebookservices_subplugin extends backup_subplugin {
    ...
    protected function define_lti_subplugin_structure() {
        ...
        $thisactivitylineitem = new backup_nested_element('coupled_grade_item', array('id'), array(
                ...
                'needsupdate', 'timecreated', 'timemodified', 'toolproxyid', 'lineitemtoolproviderid'));
        ...
        // Define sources.
        $thisactivitylineitemssql = "SELECT g.*,l.toolproxyid,l.lineitemtoolproviderid
                           FROM {grade_items} g JOIN {ltiservice_gradebookservices} l ON (g.itemnumber = l.id)
                           WHERE courseid = ?
                           AND g.itemtype='mod' AND g.itemmodule = 'lti' AND g.iteminstance = ? AND g.itemnumber>10000";
        $nonactivitylineitemssql = "SELECT g.*,l.toolproxyid,l.lineitemtoolproviderid
                           FROM {grade_items} g JOIN {ltiservice_gradebookservices} l ON (g.itemnumber = l.id)
                           WHERE courseid = ?
